...
def upload_course_material(course_id):
    ...
    cursor = mydb.cursor(dictionary=True)
    try:
        # First verify if user is a professor
        cursor.execute("SELECT Role FROM User WHERE UserID = %s", (session['user_id'],))
        user = cursor.fetchone()
        if user['Role'] != 'professor':
            return jsonify({'message': 'Only professors can upload course materials'}), 403
...
def get_all_users():
    ...
    try:
        cursor.execute("""
            SELECT UserID, Username, FirstName, LastName, Email, Role, CreatedAt 
...
# Move course creation to admin routes
...
@app.route('/admin/enrollment/requests', methods=['GET'])
def get_enrollment_requests():
    ...
    try:
        cursor.execute("""
            SELECT er.*, u.FirstName, u.LastName, u.Email,
        ...
        return jsonify({'enrollment_requests': requests}), 200
    except mysql.connector.Error as err:
        return jsonify({'message': 'Error fetching enrollment requests'}), 500
...
@app.route('/professor/assignments/<int:course_id>', methods=['GET'])
def get_course_assignments(course_id):
    ...
    try:
        cursor.execute("""
            SELECT a.*, COUNT(s.SubmissionID) as submission_count
            FROM Assignment a
...
@app.route('/professor/students/<int:course_id>', methods=['GET'])
    ...
    try:
        cursor.execute("""
            SELECT u.UserID, u.FirstName, u.LastName, u.Email,
                   COUNT(DISTINCT s.SubmissionID) as submissions_count
            FROM User u
            JOIN Enrollment e ON u.UserID = e.StudentID
            LEFT JOIN Assignment a ON a.CourseID = e.CourseID
            LEFT JOIN Submission s ON s.StudentID = u.UserID AND s.AssignmentID = a.AssignmentID
            WHERE e.CourseID = %s AND u.Role = 'student'
            GROUP BY u.UserID
        """, (course_id,))
    ...
    try:
        cursor.execute("""
            SELECT a.*, c.CourseName, c.CourseCode,
                   s.SubmissionID, s.SubmissionDate, s.Grade,
                   CASE 
                       WHEN s.SubmissionID IS NOT NULL THEN 'submitted'
                       WHEN a.DueDate < NOW() THEN 'overdue'
                       ELSE 'pending'
                   END as status
            FROM Assignment a
...
@app.route('/student/courses/drop/<int:course_id>', methods=['POST'])
def drop_course(course_id):
    ...
    try:
        ...
        return jsonify({'message': 'Error dropping course'}), 500
